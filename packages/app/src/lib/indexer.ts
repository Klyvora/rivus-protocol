...
export const indexer = {
  listStreams: (params?: { recipient?: string; sender?: string }) => {
    const qs = params?.recipient ? `?recipient=${params.recipient}` : params?.sender ? `?sender=${params.sender}` : "";
    return get<{ streams: ApiStream[] }>(`/streams${qs}`);
  },
  getStream: (id: string) => get<{ stream: ApiStream }>(`/streams/${id}`),
  getClaimable: (id: string) => get<{ streamId: string; claimable: string; withdrawn: string; totalAmount: string }>(`/streams/${id}/claimable`),
};